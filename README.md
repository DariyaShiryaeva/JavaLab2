...
Класс **`Name`** предназначен для хранения и обработки информации о человеке: **фамилии**, **имени** и **отчестве**.  
Он обеспечивает гибкое создание объекта с разным количеством параметров, проверку наличия полей и аккуратное форматирование имени при выводе.
...
| Поле | Тип | Описание |
|------|-----|-----------|
| `lastName` | `String` | Фамилия человека (может быть `null` или пустой). |
| `firstName` | `String` | Имя человека (основное обязательное поле). |
| `patronymic` | `String` | Отчество человека (может быть `null` или пустое). |
...
1. **Пустой конструктор**  
   `public Name()`  
   - Создаёт объект без указания имени. Все поля равны `null`.

2. **Конструктор с именем**  
   `public Name(String firstName)`  
   - Создаёт объект, указывая только имя.  
   - `lastName` и `patronymic` остаются `null`.

3. **Конструктор с фамилией и именем**  
   `public Name(String lastName, String firstName)`  
   - Создаёт объект с фамилией и именем.  
   - `patronymic` остаётся `null`.

4. **Полный конструктор**  
   `public Name(String lastName, String firstName, String patronymic)`  
   - Создаёт объект с заполненными всеми тремя частями имени.


### Методы и их алгоритмы

1. **Получение фамилии.** – `public String getLastName()`  
   - Возвращает значение поля `lastName`.

2. **Изменение фамилии.** – `public void setLastName(String lastName)`  
   - Устанавливает новое значение поля `lastName`.

3. **Получение имени.** – `public String getFirstName()`  
   - Возвращает значение поля `firstName`.

4. **Изменение имени.** – `public void setFirstName(String firstName)`  
   - Устанавливает новое значение поля `firstName`.

5. **Получение отчества.** – `public String getPatronymic()`  
   - Возвращает значение поля `patronymic`.

6. **Изменение отчества.** – `public void setPatronymic(String patronymic)`  
   - Устанавливает новое значение поля `patronymic`.

7. **Проверка, заполнена ли фамилия.** – `public boolean hasLastName()`  
   - Возвращает `true`, если `lastName` не `null` и не пустая строка.  
   - Иначе возвращает `false`.

8. **Проверка, заполнено ли имя.** – `public boolean hasFirstName()`  
   - Возвращает `true`, если `firstName` не `null` и не пустая строка.  
   - Иначе возвращает `false`.

9. **Проверка, заполнено ли отчество.** – `public boolean hasPatronymic()`  
   - Возвращает `true`, если `patronymic` не `null` и не пустое.  
   - Иначе `false`.

10. **Формирование строки с полным именем.** – `@Override public String toString()`  
    - Создаёт пустой объект `StringBuilder result`.  
    - Если фамилия есть — добавляет её в строку.  
    - Если имя есть — добавляет пробел (если строка не пуста) и имя.  
    - Если отчество есть — добавляет пробел и отчество.  
    - Возвращает итоговую строку в формате:  
      ```
      Фамилия Имя Отчество
      ```
      (если какие-то части отсутствуют — они просто пропускаются).


## 👤 Класс `PersonWithName`

Класс **`PersonWithName`** описывает человека, у которого есть имя (`Name`), рост и, при необходимости, ссылка на отца.  
Он реализует механизм **наследования фамилии и отчества** от отца, если у текущего объекта эти данные отсутствуют.
...
| Поле | Тип | Описание |
|------|-----|-----------|
| `name` | `Name` | Объект, содержащий фамилию, имя и отчество человека. |
| `height` | `int` | Рост человека (в сантиметрах, от 1 до 299). |
| `father` | `PersonWithName` | Ссылка на объект-отца (может быть `null`). |

---

### Конструкторы

1. **Базовый приватный конструктор**  
   `private PersonWithName(Name name, int height, PersonWithName father)`  
   - Инициализирует поля объекта.  
   - Если `name == null`, создаёт пустой объект `Name`.  
   - Если рост вне диапазона (не от 1 до 299) — устанавливает `height = 0`.  
   - Если передан `father`, автоматически вызывает `inheritFromFather()` для наследования данных.

2. **С именем и ростом**  
   `public PersonWithName(String firstName, int height)`  
   - Создаёт объект на основе имени и роста.  
   - Поле `father` по умолчанию `null`.

3. **С именем, ростом и отцом**  
   `public PersonWithName(String firstName, int height, PersonWithName father)`  
   - Создаёт объект и устанавливает отца, после чего выполняет наследование фамилии и отчества.

4. **С объектом `Name` и ростом**  
   `public PersonWithName(Name name, int height)`  
   - Создаёт объект с уже готовым именем и указанным ростом.  
   - `father` по умолчанию `null`.

### ⚙️ Методы и их алгоритмы

1. **Получение имени.** – `public Name getName()`  
   - Возвращает объект `Name`, связанный с этим человеком.

2. **Изменение имени.** – `public void setName(Name name)`  
   - Устанавливает новое имя.  
   - Если `name == null`, создаётся новый пустой объект `Name`.

3. **Получение роста.** – `public int getHeight()`  
   - Возвращает значение роста.

4. **Изменение роста.** – `public void setHeight(int height)`  
   - Проверяет, что рост находится в диапазоне от 1 до 299.  
   - Если нет — устанавливает `height = 0`.

5. **Получение отца.** – `public PersonWithName getFather()`  
   - Возвращает объект-отца, если он задан.

6. **Изменение отца.** – `public void setFather(PersonWithName father)`  
   - Устанавливает нового отца.  
   - Если `father != null`, выполняется `inheritFromFather()` для наследования данных.

7. **Наследование данных от отца.** – `private void inheritFromFather()`  
   - Проверяет, что отец существует и у него заполнено имя.  
   - Если у текущего человека нет фамилии, но у отца она есть — наследует её.  
   - Если у текущего человека нет отчества, но у отца есть имя — генерирует отчество на основе имени отца с помощью `generatePatronymic()`.

8. **Генерация отчества по имени отца.** – `private String generatePatronymic(String fathersName)`  
   - Если имя отца пустое, возвращает `null`.  
   - Если имя оканчивается на `й`, добавляет суффикс `"евич"`.  
   - В противном случае добавляет `"ович"`.  
   - Примеры:  
     - `"Игорь"` -> `"Игорьевич"`  
     - `"Павел"` -> `"Павелович"`

9. **Формирование текстового представления объекта.** – `@Override public String toString()`  
   - Возвращает строку в формате:  
     ```
     Фамилия Имя Отчество, рост
     ```

## 🏙️ Класс `City`

Класс **`City`** представляет город с возможностью хранения информации о маршрутах в другие города и их стоимости.  
Он позволяет добавлять маршруты, получать их стоимость и выводить удобное текстовое представление.
...
| `targetCities` | `ArrayList<City>` | Список городов, в которые есть маршруты. |
| `costs` | `ArrayList<Integer>` | Список стоимостей маршрутов, соответствующих `targetCities`. |

### Конструктор

1. **Создание города с названием**  
   `public City(String name)`  
   - Инициализирует объект с указанным именем.  
   - Создаёт пустые списки `targetCities` и `costs`.

### Методы и их алгоритмы

1. **Получение имени города.** – `public String getName()`  
   - Возвращает поле `name`.

2. **Изменение имени города.** – `public void setName(String name)`  
   - Устанавливает новое название города.

3. **Добавление маршрута к другому городу.** – `public void addRoute(City targetCity, int cost)`  
   - Проверяет, что:  
     - `targetCity` не `null`,  
     - `targetCity` не равен текущему городу,  
     - `cost > 0`.  
   - Если проверки проходят, добавляет город в `targetCities` и стоимость в `costs`.

4. **Получение стоимости маршрута до конкретного города.** – `public Integer getRouteCost(City targetCity)`  
   - Перебирает список `targetCities`.  
   - Если находит совпадение, возвращает соответствующую стоимость из `costs`.  
   - Если маршрут отсутствует — возвращает `null`.

5. **Получение списка всех целевых городов.** – `public ArrayList<City> getTargetCities()`  
   - Возвращает список `targetCities`.

6. **Получение списка стоимостей маршрутов.** – `public ArrayList<Integer> getCosts()`  
   - Возвращает список `costs`.

7. **Формирование текстового представления города.** – `@Override public String toString()`  
   - Создаёт строку с названием города.  
   - Если есть маршруты, добавляет `->` и перечисляет каждый маршрут в формате:  
     ```
     НазваниеГорода:Стоимость
     ```  
     Разделяет маршруты запятыми.  
   - Если маршрутов нет, добавляет `(нет маршрутов)`.


## 🐱 Класс `Cat`

Класс **`Cat`** представляет кота с именем и возможностью издавать мяуканье.  
Позволяет вывести кота в текстовом виде и заставить его «мяукать» один или несколько раз.
...
1. **Создание кота с именем**  
   `public Cat(String name)`  
   - Устанавливает имя кота при создании объекта.

### Методы и их алгоритмы

1. **Приведение кота к строке.** – `@Override public String toString()`  
   - Возвращает строку в формате:  
     ```
     кот: Имя
     ```
...
2. **Мяукнуть один раз.** – `public void meow()`  
   - Выводит в консоль:  
     ```
     Имя: мяу!
     ```
...
3. **Мяукнуть N раз.** – `public void meow(int n)`  
   - Если `n <= 0`, выводит просто имя кота с двоеточием:  
     ```
     Имя:
     ```  
   - Иначе формирует строку вида:  
     ```
     мяу-мяу-мяу-...!
     ```  
     с количеством `"мяу"`, равным `n`, разделяя дефисами, и выводит вместе с именем кота.  
   - Пример: `meow(3)` → `"Барсик: мяу-мяу-мяу!"`
